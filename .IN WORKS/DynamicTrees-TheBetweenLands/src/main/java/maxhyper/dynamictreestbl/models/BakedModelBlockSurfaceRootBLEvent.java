...
public class BakedModelBlockSurfaceRootBLEvent implements IBakedModel {
    ...
    private IBakedModel[][] coresF = new IBakedModel[2][8]; //8 Cores for 2 axis(X, Z) with the bark texture on all 6 sides rotated appropriately.
    private IBakedModel[][] vertsF = new IBakedModel[4][8];

    private IBakedModel[][] sleevesS = new IBakedModel[4][7];
    private IBakedModel[][] coresS = new IBakedModel[2][8]; //8 Cores for 2 axis(X, Z) with the bark texture on all 6 sides rotated appropriately.
    ...
    public BakedModelBlockSurfaceRootBLEvent(ResourceLocation barkRes, ResourceLocation barkResF, ResourceLocation barkResS, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        ...
        if (barkResF == null) hasFrosty = false;
        if (barkResS == null) hasSpooky = false;
        ...
        if (hasFrosty){
            ...
            for(int r = 0; r < 8; r++) {
                ...
                if(radius < 8) {
                    for(EnumFacing dir: EnumFacing.HORIZONTALS) {
                        int horIndex = dir.getHorizontalIndex();
                        sleevesF[horIndex][r] = bakeSleeve(radius, dir, barkIconF);
                        vertsF[horIndex][r] = bakeVert(radius, dir, barkIconF);
                    }
                }
                coresF[0][r] = bakeCore(radius, Axis.Z, barkIconF); //NORTH<->SOUTH
                coresF[1][r] = bakeCore(radius, Axis.X, barkIconF); //WEST<->EAST
            }
        }
        ...
        if (hasSpooky){
            ...
            for(int r = 0; r < 8; r++) {
                ...
                if(radius < 8) {
                    for(EnumFacing dir: EnumFacing.HORIZONTALS) {
                        int horIndex = dir.getHorizontalIndex();
                        sleevesS[horIndex][r] = bakeSleeve(radius, dir, barkIconS);
                        vertsS[horIndex][r] = bakeVert(radius, dir, barkIconS);
                    }
                }
                coresS[0][r] = bakeCore(radius, Axis.Z, barkIconS); //NORTH<->SOUTH
                coresS[1][r] = bakeCore(radius, Axis.X, barkIconS); //WEST<->EAST
            }
        }
                ...
                for(EnumFacing connDir : EnumFacing.HORIZONTALS) {
                    ...
                    if (connRadius > 0) {//  && (connRadius == 1 || side != connDir)) {
                        ...
                        if(extendedBlockState.getValue(BlockSurfaceRoot.LEVELS[idx]) == BlockSurfaceRoot.ConnectionLevel.HIGH) {
                            quadsList.addAll(getVerts()[idx][connRadius-1].getQuads(extendedBlockState, side, rand));
                        }
                    }
                }
            }
