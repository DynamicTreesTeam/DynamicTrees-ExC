...
public class BakedModelBlockBranchBLEvent implements IBakedModel {
    ...
    private IBakedModel[][] coresF = new IBakedModel[3][8]; //8 Cores for 3 axis with the bark texture and all 6 sides rotated appropriately.
    private IBakedModel[] ringsF = new IBakedModel[8]; //8 Cores with the ring textures on all 6 sides

    private IBakedModel[][] sleevesS = new IBakedModel[6][7];
    private IBakedModel[][] coresS = new IBakedModel[3][8]; //8 Cores for 3 axis with the bark texture and all 6 sides rotated appropriately.
    private IBakedModel[] ringsS = new IBakedModel[8]; //8 Cores with the ring textures on all 6 sides

    boolean hasFrosty = true, hasSpooky = true;

    private IBakedModel[][] getSleeves (){
        if (hasFrosty && ModelRegistry.WINTER_EVENT.isActive())
            return sleevesF;
        if (hasSpooky && ModelRegistry.SPOOK_EVENT.isActive())
            return sleevesS;
        return sleeves;
    }
    private IBakedModel[][] getCores (){
        if (hasFrosty && ModelRegistry.WINTER_EVENT.isActive())
            return coresF;
        if (hasFrosty && ModelRegistry.SPOOK_EVENT.isActive())
            return coresS;
        return cores;
    }
    ...
    public BakedModelBlockBranchBLEvent(ResourceLocation barkRes, ResourceLocation ringsRes, ResourceLocation barkResF, ResourceLocation ringsResF, ResourceLocation barkResS, ResourceLocation ringsResS, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        ...
        if (barkResF == null || ringsResF == null) hasFrosty = false;
        if (barkResS == null || ringsResS == null) hasSpooky = false;
        ...
        if (hasFrosty){
            ...
            for(int i = 0; i < 8; i++) {
                ...
                if(radius < 8) {
                    for(EnumFacing dir: EnumFacing.VALUES) {
                        sleevesF[dir.getIndex()][i] = bakeSleeve(radius, dir, barkIconF);
                    }
                }
                coresF[0][i] = bakeCore(radius, Axis.Y, barkIconF); //DOWN<->UP
                coresF[1][i] = bakeCore(radius, Axis.Z, barkIconF); //NORTH<->SOUTH
                coresF[2][i] = bakeCore(radius, Axis.X, barkIconF); //WEST<->EAST

                ringsF[i] = bakeCore(radius, Axis.Y, ringIconF);
            }
        }
        ...
        if (hasSpooky){
            ...
            for(int i = 0; i < 8; i++) {
                ...
                if(radius < 8) {
                    for(EnumFacing dir: EnumFacing.VALUES) {
                        sleevesS[dir.getIndex()][i] = bakeSleeve(radius, dir, barkIconS);
                    }
                }
                coresS[0][i] = bakeCore(radius, Axis.Y, barkIconS); //DOWN<->UP
                coresS[1][i] = bakeCore(radius, Axis.Z, barkIconS); //NORTH<->SOUTH
                coresS[2][i] = bakeCore(radius, Axis.X, barkIconS); //WEST<->EAST

                ringsS[i] = bakeCore(radius, Axis.Y, ringIconS);
            }
        }
        ...
        if (blockState instanceof IExtendedBlockState) {
            ...
            if(side == null || coreRadius != connections[side.getIndex()]) {
                if(coreRingDir == null || coreRingDir != side) {
                    quadsList.addAll(getCores()[coreDir][coreRadius-1].getQuads(blockState, side, rand));
                } else {
                    quadsList.addAll(getRings()[coreRadius-1].getQuads(blockState, side, rand));
                }
            }
            ...
            if(coreRadius != 8) { //Special case for r!=8.. If it's a solid block so it has no sleeves
                for(EnumFacing connDir : EnumFacing.VALUES) {
                    ...
                    if (connRadius > 0  && (connRadius == 1 || side != connDir)) {
                        quadsList.addAll(getSleeves()[idx][connRadius-1].getQuads(extendedBlockState, side, rand));
                    }
                }
            }
        } else {
