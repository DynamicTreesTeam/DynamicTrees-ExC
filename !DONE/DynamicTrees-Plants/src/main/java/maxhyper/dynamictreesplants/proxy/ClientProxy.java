package maxhyper.dynamictreesplants.proxy;
...
import maxhyper.dynamictreesplants.event.EventListenerPlants;
...
public class ClientProxy extends CommonProxy {
	...
	public void preInit() {
		super.preInit();
		MinecraftForge.EVENT_BUS.register(new EventListenerPlants());
	}
	...
	public void registerColorHandlers() {
		for (BlockDynamicLeaves leaves: LeavesPaging.getLeavesMapForModId(DynamicTreesPlants.MODID).values()) {
		...
		blockColors.registerBlockColorHandler((state, world, pos, tintIndex) -> { // Rooty Dirt
				...
				ModContent.rootyNetherDirt);
		blockColors.registerBlockColorHandler((state, world, pos, tintIndex) -> { // Rooty Dirt
					switch(tintIndex) {
						case 0: return blockColors.colorMultiplier(ModContent.rootyCrystalDirt.getMimic(world, pos), world, pos, tintIndex);
						case 1: return state.getBlock() instanceof BlockRooty ? ((BlockRooty) state.getBlock()).rootColor(state, world, pos) : 0xFFFFFFFF;
						default: return 0xFFFFFFFF;
					}
				},
				ModContent.rootyCrystalDirt);
	}
}
