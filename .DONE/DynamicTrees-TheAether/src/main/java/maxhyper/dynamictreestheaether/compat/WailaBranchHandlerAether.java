...
public class WailaBranchHandlerAether extends WailaBranchHandler {
    ...
    public List<String> getWailaBody(ItemStack itemStack, List<String> tooltip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        if(species != Species.NULLSPECIES) {
            ...
            if(lastVolume > 0) {
                ...
                if(las.logs > 0) {
                    ItemStack logStack = new ItemStack(ALTreeGoldenOak.logBlock);
                    ...
                    Item heldItem = accessor.getPlayer().getHeldItemMainhand().getItem();
                    if (heldItem instanceof ItemAetherTool && ((ItemAetherTool) heldItem).toolType == EnumAetherToolType.AXE) {
                        if ((heldItem instanceof ItemZaniteTool || heldItem instanceof ItemGravititeTool || heldItem instanceof ItemValkyrieTool)){
                            ItemStack amberStack = new ItemStack(ItemsAether.golden_amber);
                            String amberName = amberStack.getItem().getRegistryName().toString();
                            renderString += SpecialChars.getRenderString("waila.stack", "1", amberName, String.valueOf((int)(las.logs*1.5)), String.valueOf(amberStack.getItemDamage()));
