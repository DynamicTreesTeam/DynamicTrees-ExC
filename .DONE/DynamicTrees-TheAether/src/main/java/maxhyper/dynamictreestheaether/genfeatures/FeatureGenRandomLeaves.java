...
public class FeatureGenRandomLeaves implements IPostGenFeature {
    ...
    public boolean postGeneration(World world, BlockPos rootPos, Species species, Biome biome, int radius, List<BlockPos> endPoints, SafeChunkBounds safeBounds, IBlockState initialDirtState) {
        changeLeaves(world, rootPos, worldGenProportion, safeBounds);
        return true;
    }

    private void attemptLeafChange(World world, BlockPos pos, SafeChunkBounds safeBounds){
        if (!safeBounds.inBounds(pos, true)) return;
        if (world.getBlockState(pos).getBlock() == leaf1.getBlock() && world.getBlockState(pos).getValue(BlockDynamicLeaves.TREE).equals(leaf1.getValue(BlockDynamicLeaves.TREE))){
            safeBounds.setBlockState(world, pos, leaf2, true);
            ...
            safeBounds.setBlockState(world, pos, leaf1, true);
        }
    }

    private void changeLeaves(World world, BlockPos rootPos, float proportion, SafeChunkBounds safeBounds){
            ...
            for(int z=rootPos.getZ()-boxSize; z<rootPos.getZ()+2*boxSize; z++){
                for(int y=rootPos.getY(); y<rootPos.getY()+boxHeight; y++){
                    if (rand.nextFloat()*(1/proportion) <= 1){
                            attemptLeafChange(world, new BlockPos(x,y,z), safeBounds);
                    }
                }
            }
        }
