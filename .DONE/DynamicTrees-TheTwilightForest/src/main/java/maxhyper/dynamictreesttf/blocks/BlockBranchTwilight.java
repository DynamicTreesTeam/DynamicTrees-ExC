...
public class BlockBranchTwilight extends BlockBranchBasic {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (handStack.getItem() == Item.getItemFromBlock(TFBlocks.cicada) && worldIn.getBlockState(pos).getValue(RADIUS) == 8 && worldIn.getBlockState(pos.offset(facing)).getBlock().isReplaceable(worldIn, pos.offset(facing))){
            ...
            return true;
        } else if (handStack.getItem() == Item.getItemFromBlock(TFBlocks.firefly) && worldIn.getBlockState(pos).getValue(RADIUS) == 8 && worldIn.getBlockState(pos.offset(facing)).getBlock().isReplaceable(worldIn, pos.offset(facing))){
