...
public class BakedModelBlockBranchThickCore extends BakedModelBlockBranchBasic implements ICustomDamageModel {
    ...
    private IBakedModel[] trunksBarkFace = new IBakedModel[16];
    ...
    public BakedModelBlockBranchThickCore(ResourceLocation barkRes, ResourceLocation ringsRes, ResourceLocation thickRingsRes,  ResourceLocation barkFaceRes, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        super(barkRes, ringsRes, bakedTextureGetter);

        TextureAtlasSprite barkIcon = bakedTextureGetter.apply(barkRes);
        TextureAtlasSprite barkIconFace = bakedTextureGetter.apply(barkFaceRes);
        ...
        for (int i = 0; i < 16; i++) {
            int radius = i + 9;
            trunksBark[i] = bakeTrunkBark1(radius, barkIcon, true);
            trunksBarkFace[i] = bakeTrunkBark2(radius, barkIconFace, true);
    ...
    public IBakedModel bakeTrunkBark1(int radius, TextureAtlasSprite bark, boolean side){
        SimpleBakedModel.Builder builder = new SimpleBakedModel.Builder(modelBlock, ItemOverrideList.NONE).setTexture(bark);
        AxisAlignedBB wholeVolume = new AxisAlignedBB(8 - radius, 0, 8 - radius, 8 + radius, 16, 8 + radius);
        ...
        for (EnumFacing face: run) {
            for (Vec3i offset : offsets) {
                if (face.getAxis() == Axis.Y || new Vec3d(face.getDirectionVec()).add(new Vec3d(offset)).lengthSquared() > 2.25) { //This means that the dir and face share a common direction
                    ...
                    BlockPart part = new BlockPart(limits[0], limits[1], mapFacesIn, null, true);
                    if (face != EnumFacing.WEST || offset != Surround.W.getOffset())
                        builder.addFaceQuad(face, ModelUtils.makeBakedQuad(part, part.mapFaces.get(face), bark, face, ModelRotation.X0_Y0, false));
                }
            }
        }
    ...
    public IBakedModel bakeTrunkBark2(int radius, TextureAtlasSprite bark, boolean side){
        SimpleBakedModel.Builder builder = new SimpleBakedModel.Builder(modelBlock, ItemOverrideList.NONE).setTexture(bark);
        AxisAlignedBB wholeVolume = new AxisAlignedBB(8 - radius, 0, 8 - radius, 8 + radius, 16, 8 + radius);

        EnumFacing face = EnumFacing.WEST;
        Vec3i offset = Surround.W.getOffset();
    ...
    public List<BakedQuad> getQuads(IBlockState blockState, EnumFacing side, long rand) {
        ...
        quadsList.addAll(trunksBarkFace[coreRadius-9].getQuads(blockState, side, rand));
