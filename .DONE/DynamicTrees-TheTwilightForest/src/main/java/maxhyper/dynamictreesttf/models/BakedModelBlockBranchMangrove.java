...
public class BakedModelBlockBranchMangrove implements IBakedModel {
	...
	private static final int maxRad = 6;
	...
	public BakedModelBlockBranchMangrove(ResourceLocation barkRes, ResourceLocation ringsRes, ResourceLocation rootsRes, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
		...
		TextureAtlasSprite rootsIcon = bakedTextureGetter.apply(rootsRes);
		...
		roots = bakeRoots(rootsIcon);
		
	}
	
	private IBakedModel bakeRoots(TextureAtlasSprite roots) {
		Map<EnumFacing, BlockPartFace> mapFacesIn = Maps.newEnumMap(EnumFacing.class);
		
		BlockFaceUV uvface = null;
		uvface = new BlockFaceUV(new float[] { 0, 0, 16, 16 }, 0);
		
		mapFacesIn.put(EnumFacing.SOUTH, new BlockPartFace(null, -1, null, uvface));
		mapFacesIn.put(EnumFacing.NORTH, new BlockPartFace(null, -1, null, uvface));
		
		Vector3f posFrom = new Vector3f(0.8f, 0.0f, 8.0f);
		Vector3f posTo = new Vector3f(15.2f, 16.0f, 8.0f);
		
		BlockPartRotation rotation = new BlockPartRotation(new Vector3f(0.5f, 0.5f, 0.5f), Axis.Y, 45, true);
		
		BlockPart part = new BlockPart(posFrom, posTo, mapFacesIn, rotation, true);
		SimpleBakedModel.Builder builder = new SimpleBakedModel.Builder(modelBlock, ItemOverrideList.NONE).setTexture(roots);
		
		for(Map.Entry<EnumFacing, BlockPartFace> e : part.mapFaces.entrySet()) {
			EnumFacing face = e.getKey();
			builder.addFaceQuad(face, ModelUtils.makeBakedQuad(part, e.getValue(), roots, face, ModelRotation.X0_Y0, false));
		}
		
		mapFacesIn.clear();
		
		mapFacesIn.put(EnumFacing.WEST, new BlockPartFace(null, -1, null, uvface));
		mapFacesIn.put(EnumFacing.EAST, new BlockPartFace(null, -1, null, uvface));
		
		posFrom = new Vector3f(8.0f, 0.0f, 0.8f);
		posTo = new Vector3f(8.0f, 16.0f, 15.2f);
		part = new BlockPart(posFrom, posTo, mapFacesIn, rotation, true);
	...
	public List<BakedQuad> getQuads(IBlockState blockState, EnumFacing side, long rand) {
		...
		if (blockState instanceof IExtendedBlockState) {
			int coreRadius = MathHelper.clamp(((BlockBranch) blockState.getBlock()).getRadius(blockState), 1, maxRad);
	...
	protected int[] pollConnections(int coreRadius, IExtendedBlockState extendedBlockState) {
		...
		for(EnumFacing dir: EnumFacing.VALUES) {
			...
			connection = connection > maxRad ? 0 : connection;
