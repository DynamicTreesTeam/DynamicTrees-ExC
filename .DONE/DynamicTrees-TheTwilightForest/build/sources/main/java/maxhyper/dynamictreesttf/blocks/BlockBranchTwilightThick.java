...
public class BlockBranchTwilightThick extends BlockBranchThick {

    protected BlockBranchTwilightThick(Material material, String name, boolean extended) {
        super(material, name, extended);
    }

    public BlockBranchTwilightThick(String name) {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        int branchRadius = ((BlockBranchThick)worldIn.getBlockState(pos).getBlock()).getRadius(worldIn.getBlockState(pos));
        if (branchRadius >= 8 && worldIn.getBlockState(pos.offset(facing )).getBlock().isReplaceable(worldIn, pos.offset(facing))){
            if (handStack.getItem() == Item.getItemFromBlock(TFBlocks.cicada) ){
                ...
                return true;
            } else if (handStack.getItem() == Item.getItemFromBlock(TFBlocks.firefly)){
