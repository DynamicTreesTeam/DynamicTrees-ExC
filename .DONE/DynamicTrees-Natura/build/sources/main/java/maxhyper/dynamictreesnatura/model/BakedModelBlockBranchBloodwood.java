...
public class BakedModelBlockBranchBloodwood extends BakedModelBlockBranchBasic implements ICustomDamageModel {
    ...
    private IBakedModel[] trunksBotBark = new IBakedModel[16];//The trunk will feature bark on it's top when there's more tree on it's surface
    private IBakedModel[] trunksBotRings = new IBakedModel[16];//The trunk will feature rings on it's top when there's not any tree on it's surface(cut)
    private IBakedModel[] trunksTopRings = new IBakedModel[16];//The trunk will always feature rings on it's bottom surface(or nothing)

    public BakedModelBlockBranchBloodwood(ResourceLocation barkRes, ResourceLocation ringsRes, ResourceLocation thickRingsRes, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            trunksBotBark[i] = bakeTrunkBark(radius, barkIcon, false);
    ...
    public IBakedModel bakeTrunkBark(int radius, TextureAtlasSprite bark, boolean side) {
        ...
        final EnumFacing[] run = side ? EnumFacing.HORIZONTALS : new EnumFacing[] { EnumFacing.DOWN, EnumFacing.UP };
    ...
    public List<BakedQuad> getQuads(IBlockState blockState, EnumFacing side, long rand) {
        ...
        if (blockState instanceof IExtendedBlockState) {
            ...
            } else if (connections[0] == 1 && side == EnumFacing.DOWN) {
                quadsList.addAll(trunksBotBark[coreRadius-9].getQuads(blockState, side, rand));
            }
