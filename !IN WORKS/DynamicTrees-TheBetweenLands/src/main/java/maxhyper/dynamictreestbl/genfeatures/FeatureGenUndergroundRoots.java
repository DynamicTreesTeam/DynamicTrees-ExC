...
public class FeatureGenUndergroundRoots  implements IPostGenFeature, IPostGrowFeature {
    public BlockDynamicHearthgroveRoots rootsDirt, rootsDirtSwamp;
    public BlockBranch rootsExposed;
    public int rootBranchChance, maxRadius, rootGrowChance, rootTypeChangeHeight;
    public boolean replaceWhenFinish;


    public FeatureGenUndergroundRoots(BlockDynamicHearthgroveRoots rootDirt,BlockDynamicHearthgroveRoots rootDirtSwamp, BlockBranch root, int minHeight, int maxRootRadius, int branchChance, int growChance){
        rootsDirt = rootDirt;
        rootsDirtSwamp = rootDirtSwamp;
        rootsExposed = root;
        rootBranchChance = branchChance;
        maxRadius = Math.min(maxRootRadius, 8);
        rootGrowChance = growChance;
        rootTypeChangeHeight = minHeight;
    ...
    private boolean isSwampGroundBlock(Block block){
        return isGroundBlock(block) ||
                isSwampGrassBlock(block) ||
                block == BlockRegistry.SWAMP_DIRT ||
                block == BlockRegistry.COARSE_SWAMP_DIRT ||
                block == BlockRegistry.PEAT ||
                block == BlockRegistry.SLUDGY_DIRT ||
                block == BlockRegistry.SPREADING_SLUDGY_DIRT ||
                block == BlockRegistry.MUD ||
                block == BlockRegistry.SILT;
    }
    private boolean isGrassBlock(Block block){
        return block == Blocks.GRASS;
    }
    private boolean isSwampGrassBlock(Block block){
        return isGrassBlock(block) ||
                block == BlockRegistry.SWAMP_GRASS ||
    ...
    private boolean checkAvailableAround(World world, BlockPos blockPos, EnumFacing cameFrom){
        for (int i=0;i<6;i++){
            ...
            if(world.isBlockLoaded(blockPos.offset(dirsAll[i]))) {
                ...
                if (offsetState.getProperties().containsKey(BlockDynamicHearthgroveRoots.RADIUS)){
                    return false;
                }
            } else {
                return false;
    ...
    private EnumFacing findRandomFreeDir(World world, BlockPos blockPos, Random rand, EnumFacing cameFrom, BlockPos rootPos){
        ...
        for (int i=0;i<5;i++){
            ...
            if ((world.isAirBlock(blockPos.offset(dirsExceptUp[i])) || offsetBlock == Blocks.WATER || offsetBlock == Blocks.FLOWING_WATER || isSwampGroundBlock(offsetState.getBlock())) && checkAvailableAround(world,blockPos.offset(dirsExceptUp[i]), dirsExceptUp[i].getOpposite())){
    ...
    private boolean iterateRootGrow(World world, BlockPos blockPos, Random rand, int radius, EnumFacing cameFrom, BlockPos rootPos){
        ...
        if (isRootBlock(state.getBlock())){
            int currentRadius = state.getValue(BlockDynamicHearthgroveRoots.RADIUS);
            boolean grow = currentRadius<radius && currentRadius<8;
            radius = currentRadius;
            world.setBlockState(blockPos, state.withProperty(BlockDynamicHearthgroveRoots.RADIUS, currentRadius + (grow?1:0) ));
        } else if (world.isAirBlock(blockPos) || state.getBlock() == Blocks.WATER  || state.getBlock() == Blocks.FLOWING_WATER || state.getBlock() == BlockRegistry.SWAMP_WATER || state.getBlock() == BlockRegistry.STAGNANT_WATER){
            if (blockPos.getY() > rootPos.getY() - rootTypeChangeHeight){
                world.setBlockState(blockPos, rootsExposed.getDefaultState().withProperty(BlockDynamicHearthgroveRoots.RADIUS, radius > 4 ? radius / 2 : 2));
            }
        } else if (isGroundBlock(state.getBlock())){
            if (blockPos.getY() > rootPos.getY() - rootTypeChangeHeight){
                world.setBlockState(blockPos, rootsDirt.getDefaultState().withProperty(BlockDynamicHearthgroveRoots.RADIUS, radius>4?radius/2:2).withProperty(BlockDynamicHearthgroveRoots.GRASSY, isGrassBlock(state.getBlock())));
            }
        }  else if (isSwampGroundBlock(state.getBlock())){
            if (blockPos.getY() > rootPos.getY() - rootTypeChangeHeight){
                world.setBlockState(blockPos, rootsDirtSwamp.getDefaultState().withProperty(BlockDynamicHearthgroveRoots.RADIUS, radius>4?radius/2:2).withProperty(BlockDynamicHearthgroveRoots.GRASSY, isSwampGrassBlock(state.getBlock())));
            }
        } else {
            return false;
        }
    ...
    public boolean postGeneration(World world, BlockPos blockPos, Species species, Biome biome, int i, List<BlockPos> list, SafeChunkBounds safeChunkBounds, IBlockState iBlockState) {
        ...
        if (!world.getBlockState(blockPos.down()).getProperties().containsKey(BlockDynamicHearthgroveRoots.RADIUS)){
            boolean placed = iterateRootGrow(world, blockPos.down(), rand, 2, EnumFacing.UP, blockPos);
            if (!placed) return false;
        }
        while (world.getBlockState(blockPos.down()).getValue(BlockDynamicHearthgroveRoots.RADIUS) < maxRadius){
            int radius = world.getBlockState(blockPos.down()).getValue(BlockDynamicHearthgroveRoots.RADIUS);
            boolean grow = rand.nextInt((radius/2)*rootGrowChance) == 0;
            iterateRootGrow(world, blockPos.down(), rand, world.getBlockState(blockPos.down()).getValue(BlockDynamicHearthgroveRoots.RADIUS)+(grow?1:0), EnumFacing.UP, blockPos);
            if (replaceWhenFinish && world.getBlockState(blockPos.down()).getValue(BlockDynamicHearthgroveRoots.RADIUS) == maxRadius){
                world.setBlockState(blockPos, rootsExposed.getDefaultState().withProperty(BlockDynamicHearthgroveRoots.RADIUS, 8));
            }
        }
        return true;
    }
    ...
    public boolean postGrow(World world, BlockPos blockPos, BlockPos blockPos1, Species species, int i, boolean b) {
        ...
        if (!world.getBlockState(blockPos.down()).getProperties().containsKey(BlockDynamicHearthgroveRoots.RADIUS)){
            boolean placed = iterateRootGrow(world, blockPos.down(), rand, 2, EnumFacing.UP, blockPos);
            if (!placed) return false;
        } else if (world.getBlockState(blockPos.down()).getValue(BlockDynamicHearthgroveRoots.RADIUS) < maxRadius){
            int radius = world.getBlockState(blockPos.down()).getValue(BlockDynamicHearthgroveRoots.RADIUS);
            boolean grow = rand.nextInt((radius/2)*rootGrowChance) == 0;
            iterateRootGrow(world, blockPos.down(), rand, world.getBlockState(blockPos.down()).getValue(BlockDynamicHearthgroveRoots.RADIUS)+(grow?1:0), EnumFacing.UP, blockPos);
        } else if (replaceWhenFinish && world.getBlockState(blockPos.down()).getValue(BlockDynamicHearthgroveRoots.RADIUS) == maxRadius){
            world.setBlockState(blockPos, rootsDirt.getDefaultState().withProperty(BlockDynamicHearthgroveRoots.RADIUS, 8));
        }
        return true;
    }
}
